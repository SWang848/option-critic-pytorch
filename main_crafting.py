...
parser.add_argument(
    "--reward-shapping",
    type=int,
    default=RewardShaping.DIRECT_USEFUL,
    help=("shapping rewards."),
)

parser.add_argument(
    "--max_steps_ep", type=int, default=200, help="number of maximum steps per episode."
)
parser.add_argument(
    "--max_steps_total",
    type=int,
...
def run(args):
    ...
    logger = Logger(
        logdir=config.logdir, run_name=f"MineCrafting-{config.exp}-{time.ctime()}"
    )
    ...
    while steps < config.max_steps_total:
        ...
        while not done and ep_steps < config.max_steps_ep:
            ...
            if option_termination:
                ...
                current_option = (
                    np.random.choice(config.num_options)
            ...
            action, logp, entropy = option_critic.get_action(
                state, current_option, invalid_action_mask
            )

            next_obs, reward, done, truncate, infos = env.step(action)
            invalid_action_mask = torch.tensor(infos["action_is_legal"])
            buffer.push(
                obs, current_option, reward, next_obs, done, invalid_action_mask
            )
            rewards += reward
            ...
            if len(buffer) > config.batch_size:
                ...
                if steps % config.update_frequency == 0:
                    ...
                    critic_loss = critic_loss_fn(
                        option_critic, option_critic_prime, data_batch, config
                    )
                    loss += critic_loss
